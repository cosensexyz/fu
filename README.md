...
| `fu gc` | Safely prune completed transaction journal revisions and markers, and reclaim the bookkeeping a finished `rm`, `update`, or `fu.yaml` rewrite no longer needs; the originals an `adopt` replaced are never deleted. It also reclaims temporary payloads a killed process abandoned — a half-finished clone or source checkout — but only ones fu can prove are its own and no longer in use. Whether a holder is still alive is decided by a lock the kernel drops when a process dies, never by a recorded pid, so a download in progress is never taken out from under you — on a local filesystem, which is where that lock means what it says; put `$FU_HOME` on NFS or SMB and two machines can believe they both hold it. As always, anything whose identity no longer matches what was recorded is left exactly where it is. This is the one command that still runs when `$FU_HOME` holds no store yet, because that is precisely when an interrupted `fu clone` leaves something to collect — it sweeps the staging area and says that is all it did. |
| `fu list` | Show every skill and the full switch matrix. |
| `fu show <name>` | Show one skill's frontmatter, digest and per-agent state. |
| `fu status` | Report how `fu.yaml`'s expectations and what's on disk differ, plus the store worktree's state, any unfinished transaction, what `recovery/` and `staging/` are holding, and where your branch stands against the remote — one ls-remote, nothing written and nothing fetched. Same commit both sides means in sync; ahead, behind or diverged are only claimed when this store already holds the remote's commit and can prove it. When it doesn't, the answer is that fu cannot tell without fetching, and `fu pull` is what settles it — that is not "behind", even though behind is usually the truth, and it is not a remote failure. A remote that cannot be reached costs you that one line and nothing else; read-only, and finding a difference is not a failure — it exits 0 with the report. Failing to read the store at all is still an error. Read-only means it writes no store content, takes no store lock and creates no agent directory; the one exception is how it asks whether a temporary payload is still in use, which is to try a *shared* lock on that payload's lease for an instant — enough to tell a live holder (who holds it exclusively) from an absent one, and not enough to make one reader look like a holder to another; opening `$FU_HOME` does recreate `staging/` and `recovery/` if they have gone missing, which every command does alike. |
