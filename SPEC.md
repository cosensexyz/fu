...
| 命令 | 状态 | 说明 |
|------|------|------|
| `fu commit [name] [-m <说明>]` | 已交付 | 主动提交工作区的手工修改；带 name 仅提交 `skills/<name>/` 之下的变化，其余改动留在工作区——name 须是 `fu.yaml` 已登记的 skill，手工创建而未登记的目录以 `unknown skill "<name>"` 退出 1，此时改用省略 name 的形式即可把它一并记入；若该 skill 之外另有已用 git 暂存、且与最近一次提交不同的路径，本次提交整体拒绝并点名这些路径——它们会随本次提交一并入账，而这一笔是以该 skill 命名的——须先取消其暂存，或改用省略 name 的形式；省略 name 则提交全部——先把已用 git 暂存的快照记为一笔"外部修改"提交，再提交工作区其余变化，前者已耗尽全部差异时如实报告该外部修改，不按 nothing to commit 处理。主题行由 fu 生成（`commit: <name>`，或按变更推导的 `commit: <skill 列表>[, fu.yaml][, store]`，skill 超过五个时列表收缩为 `N skills`），`-m` 文本作为正文。两种形态在各自范围内都与 sweep 一样不加区分地入账：未跟踪的、以及被 `.gitignore` 忽略的内容照样记录（§5.3），带 name 只是把这一投影限定在该前缀之下。带 name 时以工作区为准：该 skill 之内仅经 git 暂存、其后又被编辑的中间版本不入历史——**在写出提交的那条路径上**与 `git commit <路径>` 一致，但候选树与 HEAD 同树、因而不写提交时二者相反：同样的状态下 git 拒绝提交并原样保留该暂存版本，fu 则打印 `nothing to commit` 退出 0 并把公共 index 同步到工作区，该暂存版本就此丢弃（§9 第二处不可恢复）；省略 name 的形式则先把它记为「外部修改」，不受此影响。无变化时打印 `nothing to commit` 并退出 0——带 name 时指该 skill 之下无变化；这句话说的是没有内容可记，不表示这次运行什么都没动，上述同步正发生在这条路径上 |
