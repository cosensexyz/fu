...
**其后再追加两条**：`update`（两种形状——lock-only 与内容交换，见 §6「update 流程」）、`outdated`（只读的 ls-remote 式比对，见 §2 与 §4）。
**其后再追加两条**：`commit`（不经 `run` 的独立写入口，带 name 时经 `PrepareCommitUnder` 只冻结 `skills/<name>/`，见 §4 commit 行与 §6 `fu commit` 条目）、`log`（只读；与 `revert` 共用 `WalkOperations`，序号即 `fu revert n` 的 n，见 §4 log 行）。
`pull`、`push`、`clone`、`remote`、`agent` 均未交付。（以上五条均为 SPEC §5.1 列出、但尚未注册进 `internal/cli/root.go` 命令树的命令。）

因此文中凡描述未交付部分的段落一律标注 **（设计，未实现）**，读者不必从"已知缺口"的缺省推断某处是否已经存在。
标注出现在：§2 的 `status` **store 自身远端**核对（ls-remote）、§4 的 pull / push 行、§8 测试表中依赖未交付命令的行。
...
`fu gc` 裁剪已完成事务的 revision 与终态标记，并在裁剪前重新读取、哈希和验证整条链。它先排他写入内容寻址的 `.pruned` 记录，绑定终态标记内容与有序 revision 文件名，再开始删除；恢复扫描只接受该记录明确覆盖的删除前缀，因此进程在任一次删除后中断都可由下一次 `fu gc` 继续，完成后连同 `.pruned` 记录一起移除。损坏家族的诊断始终把 revision、`.done` 与 `.pruned` 视为一个整体，人工放弃时必须把整个家族一起移出 recovery，绝不能只移走报错中最先出现的单个文件。除 journal 外，该命令还回收三类已完成事务留下的残留。其一是已完成 rm 家族在 recovery 下遗留的孤立载荷——`reclaimCommittedRemovePayload` 丢弃错误、崩溃也会留下同样的孤立载荷，故 gc 必须能独立收拾。**回收排在裁剪该家族 journal 之前，这一次序是硬性要求**：证明载荷所需的清单就写在待删的 revision 里，先裁再收就再也无从验证；回收失败则整个家族跳过裁剪，留待下一次 `fu gc` 在清单仍在时重试。**判断载荷是否已了结必须跨越处置协议的两个名字**（第二十二轮修复）：处置不是一次 syscall——`RemoveOwnedTreeAt` 先清空树、再把根退休到按清单派生的确定性兄弟名、最后才 unlink，故后两步之间的崩溃会腾出载荷名，而把已清空的根留在 `.fu-retired-dir-<token>` 上。只 stat 载荷名即答"已了结"，裁剪就会据此删掉承载该清单的 revision，而清单正是 `RemoveOwnedTreeAt` 从那个兄弟名续跑的唯一凭据；此后该退休根再无任何东西收得走，`fu status` 却仍按前缀把它计为可回收，恰好是分桶本要避免的"让用户运行一条命令、看着计数不动"。该判断因此交给 store 的 `RecoveryPayloadSettled` 同时核对两个名字——退休名的派生留在 store 内部，日后协议再添中间名也不会把调用方甩下。只有未完成事务集读取失败那条分支会直接询问载荷，正常路径本就经 `ReclaimRecoveryPayloadOwned` 自行续跑，不受影响。**install 家族没有对应的孤儿窗口，故 gc 不为它们设回收分支**（第二十二轮记录，防再查）：`new` / `add` / `adopt` 的归档一律排在 `ClearTxn` 之前且失败即返回（`rollBackUncommittedInstall` 与 `committedInstallRecovery.finish`），无载荷那条分支更把"载荷不存在"直接写成清 WAL 的前置条件，因此一个已完成的 install 家族永远不会遗留 `rollback-*`：它要么已经在 `.fu-archive-*` 上，要么其事务根本尚未完成、名字仍被未完成记录所主张。这与 `rm` 的"回收排在 `ClearTxn` 之后"（§6 rm 流程第 4 条）恰好相反，而两者都是有意的——`rm` 的载荷在 WAL 仍开时正是回滚要还原的那份内容，不能先收；install 的载荷则是回滚已经产出的副本，不先归档就不能宣告事务结束。据此，未被认领的 `rollback-*` 只可能来自人工按 `addRecoveryConflictRemedy` 的放弃建议移走整个家族，清单已失，确实无人可收。其二是 `completeConfigExchange` 未能就地收掉的配置交换记录、终态与归档（见 §6"fu.yaml 写入"一条），它们不属于任何事务 journal，故每次运行整体回收一次，其失败只记入诊断、不阻断 journal 裁剪。这一遍回收按名字前缀进行，只碰三类名字：已带终态标记的记录、记录已消失的标记、以及名字自述的 identity 仍能对上的归档；并且只要还有任一未完成的交换记录在，归档一律不动——那份归档可能正是恢复即将收敛过去的唯一副本。**载荷名加上一份对得上的清单并不构成所有权**：`removed-<name>-<StartHead>` 在 rm 家族之间并不唯一，而从 skills 根到 recovery 的每一跳都是 rename，设备号、inode 与内容因此一路不变；于是一个已完成家族的清单可能与另一个仍未完成家族的现存载荷逐项相等，照此匹配删除会毁掉那条未完成事务的载荷，并让其后每条写命令都卡在恢复入口。故 gc 先一次性收集全部未完成事务所主张的载荷名（`pendingTxnClaims`，同一次 journal 读取里连 staging 名一并取出）并整体排除；读取这些记录不等于恢复它们，该命令仍然刻意不调用 `RecoverPending`。**其三是已完成 update 家族在 `staging/<name>` 下遗留的孤立载荷——被替换的旧树**（本轮记录）：这一类不落在 recovery 下——SPEC 规则 3 承诺被替换内容留存于 store 自己的 git 历史，disposal 因此是丢弃而非归档，staging 只是它被丢弃前的落脚点。正常路径下这份残留由该操作自身的 `afterTxnCleared` 回调（`reclaimExchangedUpdatePayload`，在 `ClearTxn` 之后、同一持锁临界区内同步执行，见 §6「update 流程」）就地清空；`reclaimExchangedUpdatePayload` 丢弃错误、崩溃也会留下同样的孤立载荷，gc 因此才会在 staging 遇到东西可收，判断与回收都交给 `reclaimUpdateStagingPayload`——该操作自身的内联回收与 `fu gc` 的补收共用的同一个原语。**未完成事务集读取失败时另有一条退化分支**，与 rm 载荷在同一情形下的那条对称，此处按 rm 一侧的详略一并写明：此刻无从证明任何 staging 名未被主张，故先问 `updateStagingPayloadSettled`——它同时核对活动名与退休兄弟名，两个名字都空即为"已了结"，该家族已无物可收、也就无清单可失，journal 照常裁剪；只要任一名字上还有东西，就整族跳过，留待下一次 `fu gc`。"了结"为何必须跨越两个名字，理由与 `RecoveryPayloadSettled` 的注释相同（处置不是一次 syscall，见本节前文），`updateStagingPayloadSettled` 即是它在 staging 一侧的对应物。**`staging/<name>` 的归属证据比 `removed-<name>-<StartHead>` 更弱，不是更强**（本轮修正）：它就是裸的 skill 名，用户自己的目录、被人工放弃的安装留下的 staged 树、以及某条未完成事务正在使用的 staged 内容，都同样合法地落在这个名字上。`new`、`add`、`adopt`、`update` 在该名字被占用时的前置拒绝，证明的是**那条未完成事务开始时这个名字是空的**，而不是此刻坐在这里的东西属于谁；早先按相反方向读这条论证，让 gc 去动一条未完成 update 事务的 staged 树，并在报错里劝用户把它移走——而那正是 `restoreExchangedUpdate` 换回去所需的唯一一份磁盘副本，移走之后恢复永远失败，其后每条写命令都卡在恢复入口。故 gc 对 staging 名同样整体排除未完成事务所主张的名字（`pendingTxnClaims`，派生规则经 `pendingStagingClaims` 与 `fu status` 共用，两边不可能再各写一份而分歧）。**名字被主张时，gc 既不碰那棵树、也照旧裁剪该家族的 journal**：不裁剪会让 journal 随未完成事务一直堆积，且 `fu status` 会把这些文件计为可回收而 gc 每次走过不动，恰是本轮要消除的分歧——与 rm 载荷被主张时的处置完全一致。**判断到主张为止，不再往下问对象本身**：本轮曾试过"活动名上的东西若仍持有该家族清单所记的根 identity（`StagingRootMatches`）就照收"，理由是主张者 stage 出的是新拷贝、inode 是新的——这条理由是错的，且错得有破坏性。交换是 rename，rename 保留 inode：一次回滚的 update 由 `restoreExchangedUpdate` 把旧树换回 `skills/<name>` 后 `ClearTxn`，留下的已完成家族其 `PreviousPayload` 记的正是**现行** `skills/<name>` 的 inode；下一次 update 再把这同一个对象换到 `staging/<name>`，若在交换与 commit 之间中断，那里的东西就与旧家族的清单在 identity、mode 与内容上逐项相等——因为本就是同一个对象——而它同时是那条未完成事务回滚时唯一能换回去的副本。收掉它，恢复将永远无法到达终态，其后每条写命令都卡在恢复入口。这正是本节前面那条规则在 staging 一侧的同一情形：**名字加上一份对得上的清单并不构成所有权**。

**代价是一处已知限制，记在此处**：`pendingStagingClaims` 有意从每条未完成记录里取 `record.Name`，不问操作类型，而 `fu rm` 根本不 stage（前置检查只看 `fu.yaml` 与 `skills/<name>`）。于是"一次 update 完成但内联回收没走完，随后同名 `fu rm` 中途中断"这条双故障路径上，未完成的 rm 会主张一个正放着该 update 家族自身孤立载荷的名字。**真正丢掉这棵树还需第三个条件**：`fu gc` 恰在那条未完成 rm 被恢复之前运行——此时 gc 留树、裁 journal，此后这棵树再无人能验证或回收，`fu status` 把它计入 `Unmatched`。先跑任一条写命令则不会：写命令在做任何事之前先恢复该 rm，rm 一旦进入终态就不再主张任何名字，下一次 `fu gc` 照常按 `default` 分支把这棵树收走。故此路径是双故障加一段时间窗，且被替换的内容仍在 store 的 git 历史里（SPEC 规则 3）；另一边则是卡死整个 store。本文件通篇的不对称由此定案：**多收一个名字只是少删一次，少收一个名字就删掉了另一条事务仍指望的内容**。退休兄弟名不进主张比对：主张集合里只有 skill 名与 `StagingReservation` 名，而 skill 名不能以 `.` 开头，故它永远不可能被主张。名字未被主张、内容却对不上清单时 gc 仍然拒绝并保留 journal（可能是 fu 自己的孤儿坏了一个字节），但诊断不再劝用户"移走以放弃这份副本"——fu 刚刚证明的恰恰是这份内容不是它的。**回收同样排在裁剪该家族 journal 之前**：能证明 staging 处内容身份的清单（该家族记录里的 `PreviousPayload`）就写在待删的 revision 里，先裁再收就再也无从验证，与 rm 载荷的次序要求出于同一原因；回收失败则整个家族跳过裁剪，留待下一次 `fu gc` 重试。回收掉的这棵树不计入 gc 输出的文件数——它是一棵按清单整体删除的目录树，与 rm 那棵同构，而 rm 那棵本就不计（见 `PruneOutcome`）。`fu status` 用 staging 一侧新增的 `Collectable` 桶报告它，与 recovery 一侧的同名桶分属两处独立计数；活动名只有在其 identity 仍与清单所记的根一致时才计入该桶（`StagingRootMatches`，只 stat 不读内容，与 `CollectableConfigArchiveNames` 同一种做法），退休兄弟名由该 identity 派生，故无须再问它自己的身份；它入桶与否分三档：名字被未完成事务主张、活动名上坐着对不上清单的东西、或 staging 目录本身列不出来，这三种条件下两个名字都不入桶；活动名上仍是本家族自己的树时，两个名字一同入桶；活动名已空时则只报出退休兄弟名——那正是 `RemoveOwnedTreeAt` 续跑的起点。不入桶的三种条件里，前两种 gc 也一个都不碰，第三种则不然：gc 没有「列不出来」这一说，若那个名字确实是空的，它会照 `default` 分支收走。此处刻意只少报不多报——列表读不出来只是证据缺失，而 `status` 本就为此退出 1。**状态侧同样按「记录是否声明了自己的载荷清单」而非「主张是否可知」决定 journal 的承诺**（`claimsKnown`，`status.go`）：未完成事务集读不出来时，本就没有载荷的家族（`new` / `add` / `adopt`）与已带 `.pruned` 记录的家族照旧计入 `Collectable`——gc 在同一情形下也照旧裁剪它们；被撤回的是最新一版 revision 声明了 `Payload` / `PreviousPayload` 的 rm 与 update 家族，**不问那个名字上此刻是否真有东西**。gc 在同一分支会去问一次文件系统，故一条内联回收已经收干净的普通 rm 在此被撤回、在 gc 那边照收——这是刻意的少报：此时 `status` 本就退出 1，少报只多花一次重读，多报则是"跑一条命令、看着计数不动"。这同样是一条用户可见的计数规则、且有专门的测试，故与 `stagingBlocked` 一并写明。staging 目录下的其余条目另由 staging walk 自身的 switch 分派：被未完成事务主张的落入 `Blocked`，认得出的残留前缀落入 `Uncollectable`，都不匹配的才落入 `Unmatched`。**一个已了结的 update 家族，其 staging 回收被外来条目挡住、或 staging 目录本身列不出来时，它自己的 journal 文件从 `Collectable` 改记入 `Uncollectable`**（`stagingBlocked`，`status.go`）：gc 的回收排在裁剪之前，回收失败即整族跳过裁剪，故这些文件此刻确实收不走，报告不得承诺它们。本节总则「以 `fu gc` 自身的判据为准」已在精神上涵盖这一条，但它是一条用户可见的计数规则、且有专门的测试，故在此写明，不只留在 Go 注释里。**该比对只在报告里用，且只回答"这是不是 fu 自己的树、而非另一个占了同名的目录"，它不是所有权判定，任何删除都不得据它决定**（理由见上；`StagingRootMatches` 自身的注释也写着这一条）——所有权由主张决定，而主张在这个 switch 里排在它前面。未完成事务、adopt 归档载荷（`adopt-archive-*`）、`new` / `add` 回滚出的隔离载荷及其归档（`rollback-*` 与 `.fu-archive-*`）、`adopt-link-*.json` 与 adopt 中断留下的孤立载荷仍不在删除范围内。link archive 在退休 symlink 前成为恢复 authority；崩溃若发生在记录落盘与 journal append 之间会留下内容寻址的孤立记录。当前无法在并发写入下证明这类记录已无恢复用途，故有意保留而不自动收集。sibling 备份等现场残留只是事务载荷，仅在与记录匹配时才被处理。（早期设想中 update 会把旧目录改名为 `.old`，此处原先据此把 `.old` 列为一类残留；已交付的实现走一次原子 exchange，`.old` 这一类现场对象根本不存在，见 §6 与本节 update 一条。）除只处理已完成事务的 journal 与残留、刻意不调用 `RecoverPending` 的 `fu gc` 外，所有写命令取得锁后的第一步是按记录恢复未完成事务，终态为三者之一：**完成、回滚、安全冲突**（现场在崩溃后被外部改动时如实报告，不强行收敛）；恢复完成前不执行普通 reconcile，sweep 不把与记录匹配的事务残留当外部修改。`status` 只读报告未完成事务，呈现为 `PendingOperation`（操作与对象名）。由此，任何中断后的下一次普通写命令或 `fu restore` 都到达定义的终态，用户无需知道崩溃发生在哪条命令。

## 3. fu.yaml schema

`source` 字段已实现（plan 2）：`add` 写入 git（type/url/ref/ref_kind/commit/subdir）与 local（type/path[/subdir]）两种记录，
`show` 显示来源与锁定 commit 短哈希；默认分支固定与 tag / branch 锁定均已实现，`update` 对记录的消费同样已交付（见 §6「update 流程」）。
...
- **`source` 可省略**：`fu new` 与真实目录收编的 skill 无上游：`outdated` 单列一行如实说明（`NoUpstream`、Reason 为"no source record"），`update` 的批量则连提示都不给——`selectUpdateTargets`（`application.go`）按 `NoUpstream` 把这类条目整条略过，既不计入更新、也不列入 `Unjudged`，与 `!Updatable` 的行同样保持沉默：把「本来就没有上游」写成「无法判断」，等于每次运行都在 `skipped:` / `failed:` 共用的那条通道上为一个永不改变的事实打一行噪音（第四轮修正）；symlink 收编的条目在原目标路径唯一时记录其为 local 来源，多处路径不一致则省略并警告（见 AdoptPlan）；
- local 来源的 `path` 为绝对路径，随 store 同步到其他机器后仅作提示信息——该来源只在路径存在的机器上可更新（SPEC 规则 9）。路径不存在时由 `outdated` 逐项报告"local source unreachable"，路径本身是一条 symlink 时另行拒绝并指出改记其指向的目录——`update` 以 `O_NOFOLLOW` 打开该路径（`openPreparedRoot`，`internal/source/git.go`），判断层若跟着链接走下去，就会每次都报可更新而每次 update 都失败（`judgeLocalUpdate`，`internal/engine/outdated.go`），`status` 不做这项核对：SPEC 规则 9 原先把该提示写在 `status` 名下，而 `StatusReport` 没有任何来源侧字段，本轮据 §2.2 的分工把规则 9 改为指名 `outdated`，两处就此一致。

## 4. git 封装（go-git 边界决定）

| 操作 | 实现 | 说明 |
|------|------|------|
| commit | 私有 stage + 冻结候选 + CAS 发布 | `PrepareCommit` 先在短暂持有 `.git/index.lock` 时只读捕获公共 index 基线，随后在共享真实对象库、但 `Index` / `SetIndex` 完全落于内存副本的私有 repository 中只 stage 一次，冻结由路径、Git mode 与 blob hash 组成的完整树；直接 Git 在验证、WAL 追加与发布期间始终看不到 fu 的临时候选。commit 直接从冻结条目递归构造 tree 与 commit 对象，分支只以捕获的旧引用做 CAS 更新（§6"命令提交候选冻结"）。发布成功后仅当公共 index 的完整结构仍精确等于捕获基线，才在重新取得 `.git/index.lock` 后把它同步到已提交候选；**全库候选另要求该基线在准备时与 HEAD 同树，作用域候选不要求**——后者即使基线与 HEAD 不同也照样同步，见本行末尾 `PrepareCommitUnder` 一段。同步在两处发生：正常发布之后，以及候选树与 HEAD 同树、因而不写提交的提前返回处（否则「暂存后又改回已提交内容」这类不移动树的状态永远同步不到，`fu status` 会永远报该 skill 待提交）；期间到达的直接 Git index 内容原样保留。候选被放弃不需要还原公共 index，因为准备从未写入它。他人已持有锁时 fu 与 git 一样停下并报出锁路径。提交信息格式 `<操作>: <对象>`，如 `new: pdf-tools`、`disable: writing --agent codex`、`external: manual modifications`；`fu log` 的展示直接以此为素材。`PrepareCommitUnder(prefixes)` 是同一原语的限定路径形态：对前缀执行 go-git 的目录 add（含状态可见的删除），补扫前缀下被忽略的文件，再把基线中已从磁盘消失的前缀内条目移出候选；前缀之外的路径保持公共 index 的状态，候选的变更集经断言不越出前缀。该变更集跳过 go-git 状态中 Staging 为 `Unmodified` 的路径；`Untracked` 只在该路径同时不在 HEAD 树中时才跳过。go-git 的 index→worktree 一遍会把 HEAD→index 一遍已判定的删除覆写成 `Untracked`，因此该值只说明「不在 index 中」：同时不在 HEAD 中的是全新文件，本就不属于候选要提交的树；在 HEAD 中的则是真实的 HEAD→index 删除，即 `git rm --cached` 之后文件仍留在磁盘上的状态，必须计入变更集。`PrepareCommit` 因 `stageAll` 先整体 stage 而看不出这一条的差别，两个方向的偏差都只落在 `PrepareCommitUnder` 刻意只部分 stage 的 index 上：一概不跳过，则 store 内任何一个未跟踪文件都会撞上上述断言（末轮评审第 1 条）；一概跳过，则真实的暂存删除不进变更集，断言看不见它，候选会把该路径静默移出提交的树（`git rm --cached fu.yaml` 之后 store 自认未初始化）。 |
...
**基线三态判定**：`本地修改 = digest(store) ≠ 记录基线`（sweep 使 worktree 常态干净，`worktree.Status()` 不能承担此判定）；`outdated = digest(source) ≠ 记录基线`（local 来源；git 来源按 `ref_kind` 分派，仅 branch 以 ref 头 ≠ 锁定 commit 判定，tag 与 commit-pinned lock 不参与 outdated）。两者各自独立判定、互不参照：store 侧偏离基线即走冲突分支拒绝（`--force` 覆盖），这一判定只比较 store 与基线，从不比较 source 与 store 的内容是否恰好相同——即便两侧偏离后内容彼此相同，仍按本地修改处理。source 一侧偏离而 store 未偏离时才是正常的内容更新，两种形状与交换见 §6「update 流程」。**拒绝时的"提示差异"（SPEC §5.1、规则 3）落在摘要对与 git 命令上**：基线记的是摘要而非清单，本层因此无从算出逐文件差异；拒绝信息给出记录基线与 store 当前摘要（各截短显示），并指向 store 自身的 git——改动要么仍未提交在工作区，要么已被下一条写命令 sweep 成一笔独立提交（SPEC §5.3），两条命令都给出。两处拒绝（`selectUpdateTargets` 的点名拒绝与 `checkUpdateAvailable` 的内容形状拒绝）共用 `describeLocalModification`，措辞不会分叉。
...
- **store 内不可读的文件会挡住每一条写命令（`fu commit <name>` 例外）**：作用域提交只读前缀之内的文件，故前缀之外的不可读文件不挡它——这是它优于全库形式之处，记在此处以免被当作疏漏。以下就全库形式而言：入库必须读取 store 内的每一个文件，因此一个 `chmod 000` 的文件会让所有写命令在 sweep 阶段失败。这与系统 git 一致——`git status` 容忍不可读的未跟踪文件，`git add -A` 则以 "unable to index file" 失败——所以行为本身不可约减；已做的是把裸 errno 换成点名完整路径并给出处置办法的消息（`explainStagingFailure`）。判定"是否有变更"一侧不需要读权限，已修好（`statEntryNoFollow` 从分类用的 `fstatat` 直接构造 `FileInfo`）；
...
- **`fu commit`**：不经 `run` 的独立入口（`CommitOperations`，与 `RevertOperations` 同形：锁 → 恢复 → `CheckWritable` / `CheckCanonicalPath` → 提交 → `reconcileChecked`），刻意不做整体 sweep。带 name：`cfg.HasSkill` 校验后 `PrepareCommitUnder(skills/<name>)`——该候选对前缀之外的路径取公共 index 的状态，故用户先用直接 git 暂存了前缀之外的路径时，§4 所述「变更集不越出前缀」的断言必然触发：这是用户可达的分支，不只是防御。`explainCommitScopeViolation`（`internal/engine/commit.go`）以 `errors.As` 取出 `store.CommitScopeViolationError` 携带的全部越界路径，一次点名全部而非逐条拒绝，给出在 store 目录下取消暂存的完整命令与「改用省略 name 的形式」两条出路，并以 `commitScopeRefusal` 的 `Unwrap` 保留原错误——不用 `%w`：该动词在保留错误链的同时会把被包装错误的文本一并拼进消息，而那段文本正以同一份路径清单结尾，越界路径会因此被渲染两次；省略 name：先把公共 index 的暂存快照以 `external` 提交（`PrepareStagedSnapshot`），再 `PrepareCommit` 整个工作区。主题行由 `commitSubject` 生成，`-m` 入正文；带 name 时以工作区为准——前缀之内仅经 git 暂存、其后又被编辑的中间版本被同步覆盖且不入历史。同步有两处：写出提交之后，以及候选树与 HEAD 同树、因而不写提交的提前返回处（§4 commit 行）。前一处与 `git commit <路径>` 一致；**后一处恰与之相反**——同样的状态下 git 拒绝提交并原样保留该暂存版本，fu 报 `nothing to commit` 退出 0 而把它同步掉，这是 SPEC §9 第二处不可恢复的确切落点。两处同步都会丢掉该暂存版本，后一处特殊之处不在于「只有它会丢」，而在于 git 在同样的状态下本会保住它（省略 name 的形式仍先以外部修改记入，不受此影响）；候选为空仍照常 `reconcileChecked` 修复被手工破坏的 agent 链接，不写提交、退出 0——但**必须仍然调用 `CommitPrepared`**，不可因变更集为空而短路：上述第二处同步就发生在它的无变更返回处，短路掉它，「暂存后又改回已提交内容」的状态便永远同步不到，`fu status` 会永远报该 skill 待提交（这正是评审第二轮的缺陷，当时 store 层已修好而本调用方仍短路）。今天只有 `commit.go` 的一条注释守着这一点；`CommitOutcome` 以 `Written` 与 `ExternalWritten` 两个独立字段分别记录两层是否落地、互不隐含——省略 name 时若一层已发布而另一层随后出错，CLI 仍先如实打印已落地的那层，再返回错误；
...
4. **恢复终态**：operation commit 已存在且校验通过 → 清 WAL，随后回收隔离载荷（**完成**）；否则内容按清单还原回 skills 根、配置复原、清 WAL（**回滚**）；现场与清单不再全量相等则**安全冲突**。**回收严格排在 `ClearTxn` 之后，这一次序是硬性要求**：WAL 仍打开时回收，会毁掉回滚分支要还原的那份内容；排在终态标记之后，它就永远不构成任何恢复的前置条件——此处崩溃或回收失败都只留下无人等待的孤立载荷，交由 `fu gc` 独立收拾（§2）。被删内容本身仍可从 git 历史取回，载荷只是第二份副本，故丢弃它无需自带防篡改检查，失败也绝不把一条已经持久成功的 rm 报成失败。**该处理器并不在未崩溃的命令路径上**：`pipeline.go` 的 `run()` 在 commit 落地后自行清除 WAL，`finishCommittedRemove` 因此只在崩溃恢复时被调用。为使普通 `fu rm` 走完全相同的次序，`Op` 增设默认为 nil 的 `afterTxnCleared` 回调，由 `run()` 在自身临界区内、`ClearTxn` 成功后立即触发；它仍持有本命令已取得的 `fu.lock`，无需为回收另开一次持锁会话（对一条已经成功的 rm 而言，那在用户看来就是卡死）。该回调没有错误返回：触发时操作已持久成功，其结果不容再被改变；目前 `rm` 与 `update` 设置它（update 一侧见 §6「update 流程」与 §2 的 staging 孤立载荷一条）。**回滚一侧没有补偿 commit，也不需要**：该分支只在 `currentHead == startHash` 时进入，即命令的 commit 尚未发布，没有任何已入库状态需要抵消（`finishCommittedRemove` 的注释即为此说明）。这与 `new` / `add` 的安装流程不同——那里的 `rollBackCommittedInstall` 确有 `compensation-ready` / `compensation-committed` 两阶段——不要按那份记录为 `rm` 补上它并不需要的机制。`rm` 已不再调用归档原语，"载荷已归档、WAL 未清"这一第八轮曾令恢复永久卡死的窗口随之消失：载荷一直停在隔离名上，直到 WAL 清除后才被回收，而回收原语对已经不在的载荷不报错，故恢复照旧不另做前置检查（第八轮的教训见下方"两条恢复/报告经验"；归档原语对活动名与归档名的双位置幂等仍是 `new` / `add` 侧的现行约束）。

### update 流程

**已实现（本轮）**。`fu outdated` 与 `fu update` 共享同一判断层：`Outdated`（`internal/engine/outdated.go`）产出的 `UpdateStatus` 既是 `outdated` 的展示依据，也是 `update` 批量选择的依据，二者因此不会对"是否可更新"给出不同答案；`update` 本身的事务实现在 `internal/engine/update.go` 与 `update_txn.go`。

**两种形状**：`updateSkill` 在写锁之外、`run` 之前，用调用方已经准备好的 source（`p *source.Prepared`，取得方式与 `add` 相同——写锁内绝不做网络 I/O）投影出的摘要，与 `cfg.Digest(name)`（安装或上一次更新时记录的基线）比较。摘要比较之外还有一条先行分支：**`--force` 且 store 副本已偏离基线时，一律走 content 形状，不问上游是否前移**（`update.go` 中 `storeCopyMatchesBaseline` 的判定）。理由见 §4 规则 3 一段——上游未动时若仍按摘要相同落到 lock-only，`--force` 就什么都覆盖不了，而它的帮助文本恰恰承诺覆盖本地改动；§4「基线三态判定」一段只把 `--force` 写成"覆盖拒绝"，并未说明它同时决定形状，此处补齐。其余情形按摘要二选一：

- **lock-only**：摘要相同——上游 ref 前移了，但该 skill 所在 subdir 没变。磁盘上什么都不用动，只推进 `fu.yaml` 里的来源锁定字段（`cfg.SetSourceFields`）。这一路径不开事务：给它一份 `Txn` 记录只会在恢复 journal 里放一条恢复无事可做的条目，形状因此与 `setGlobalTracked`（`ops.go`）一样，只是一个仅动 `fu.yaml` 的 `Op`。这一形状也是 `outdated` 只读、从不取得内容的直接后果（见 §4「git 来源的取得」）：它只能比对 ref 头与锁定 commit，上游提交推进而 subdir 未变时无法从远端分辨内容其实相同，只有真的 `update` 一次才能发现——不推进锁定的话，这个 skill 会被永远报告为可更新；
- **content**：摘要不同，交给 `updateSkillContent`：staging 一份新树，再用一次原子 exchange 把它和已发布的 skill 互换。

**交换决定这次更新能否撤销**：`Mutate` 把新内容 stage 到 `staging/<name>`；`Publish`（`run` 内固定排在 commit 之前，见 §6 开头流水线图）调用 `store.ExchangeStagedWithSkillOwned(name, staged, published)`，一次 rename-exchange 把 `staging/<name>` 与 `skills/<name>` 的内容原地对调。交换之前发生的一切都可以靠丢弃 staging 撤销，交换之后的一切都只能靠再交换回去撤销。交换前，`Mutate` 已分两步把双方的清单先后写进事务记录：先是这次要替换的旧树（`checkUpdateAvailable` 返回的 `store.OwnedTree`，记作 `txn.PreviousPayload`，`updateTxnSnapshotted` 阶段），staging 完成后再是新内容的清单（`txn.Payload`，`updateTxnStaged` 阶段）；交换后再补一条 `updateTxnExchanged` revision。`renameExchange` 保留两侧各自的 inode，两份清单因此在交换前后分别精确对应 `skills/<name>` 与 `staging/<name>` 的内容——恢复据此靠**比对清单**、而不是靠比对阶段标记，就能判定交换到底发生没发生：事务在崩溃前只把交换动作记进 journal，而不是先记后做，一次崩溃可能留下一条声称"未交换"的记录描述一次已经发生的交换，只有清单不会说谎。

**决策表以代码注释为准**：恢复只需回答两个问题——commit 是否已经落地、`skills/<name>` 现在装的是哪一份清单；两问交叉出的状态、各自的处置、以及哪一种状态不可达，完整一份写在 `recoverUpdate` 的文档注释里（`update_txn.go`），**此处不再复制**。这张表此前同时存在于三处（本节、设计稿 §4.6、以及该注释），并在一轮之内于本节走样——本节曾写「可达的只有四种」而漏掉「已提交但两份清单都不匹配」这一可达状态，恰是注释后来修正掉的那处错误。故本节只保留读者需要的两条断言，其余委诸注释：

- **判定依据是比对清单，不是记录里的阶段标记**：`renameExchange` 保留 inode，清单认的是**对象**而非名字，故交换前后两个名字各自持有哪一份清单是相反的；update 只在交换**之后**才记录该动作，一次崩溃可能留下一条自称"未交换"的记录描述一次已发生的交换，只有清单不会说谎；
- **无凭据即不动手**：`skills/<name>` 与两份清单都对不上时，一律拒绝并保留 WAL 交由用户处理——两个可能的动作（丢弃或换回）都是破坏性的。这一行与 commit 是否落地无关：已提交之后被 fu 之外的东西写入，同样落在这里。

**残留处置**：交换把被替换的旧树留在 `staging/<name>`，这是一份孤立载荷，与 `rm` 在 recovery/ 下留的孤立载荷同构——都以 `OwnedTree` 清单背书、都等一次回收——只是落脚点与去向不同：SPEC 规则 3 承诺这份内容仍可从 store 自己的 git 历史找回，它的归宿因此是丢弃而非像 `rm` 那样归档进 recovery/，staging 只是丢弃前的落脚点。正常路径下这一步由操作自身的 `afterTxnCleared` 回调（`reclaimExchangedUpdatePayload`）在 `ClearTxn` 之后、同一持锁临界区内同步完成，不需要 `fu gc` 介入；`reclaimExchangedUpdatePayload` 丢弃错误、崩溃也会留下同样的孤立载荷，这两种情形才轮到 `fu gc` 补上——`reclaimUpdateStagingPayload` 是这次操作自身的内联回收与 `fu gc` 的补收共用的唯一原语，回收次序与理由记在 §2「`fu gc` 裁剪」一条的"其三"。
...
- **中断产生的孤立 payload 尚无状态报告与 GC（第十七轮记录；rm 一半已关闭，adopt 与 source scratch 仍开放）**：**rm 一半已关闭**——已提交 rm 的隔离载荷在 `ClearTxn` 之后就地回收，此处崩溃或回收失败留下的孤立载荷由 `fu gc` 在裁剪该家族 journal 之前、凭同一份清单收走，并整体排除任何未完成事务所主张的载荷名（§2）。**仍开放的两半**：adopt 归档复制在进程退出窗口可能留下尚未进入完整事务所有权状态的 payload；git source scratch 的正常关闭与路径替换已按 identity 安全处理，但进程在 `Close` 前退出仍可能留下 owned 或 quarantine `.fu-src-*`。这两类仍不自动猜测归属或删除。**报告的一半已关闭（第二十二轮）**：`fu status` 现盘点 `staging/`，分四桶——分桶大多只比名字，唯独第一桶的活动名要额外问一次 identity（`StagingRootMatches`，只 stat 不读内容），见 §2——`fu gc` 会来收的（已完成 update 家族留下的孤立载荷，见 §2「其三」）；待某次恢复收尾的（未完成事务的已发布 staged 根与其私有 reservation，未完成配置交换的候选与活动交换名）；今天无人回收的（`.fu-src-*` 三种形态、`.fu-retired-staging-*`、无记录背书的 `.fu-new-*` 与候选、以及同样无记录背书的固定名 `.fu-config-swap`）；以及 fu 没有任何待处理记录可与之对应的条目（`Unmatched`，即前几类都不匹配的剩余名字）。最后一桶单列而不并入「无人回收」，是因为它确有补救、只是不由 fu 执行：落在 skill 名上的那些会让 `fu new`、`fu add`、`fu adopt` 与 `fu update` 拒绝复用并点名该路径，而刚被拒绝的用户跑 `fu status` 本应看得见挡路的是什么。这条补救以「名字是 skill 名」为条件，报告的措辞也照此写：该桶兜的是前几类都不匹配的**任何**名字，未必是公开名——双故障在 staging 下搁浅的 `.fu-retired-dir-<token>` 就落在这里，而没有哪条命令会去复用那种名字。第一桶原先没有，因为 `fu gc` 曾经从不查看 staging；`fu update` 交付后不再如此。staging 仍没有「按设计保留」一桶，因为此处没有 SPEC §9 承诺保留的 authority。**GC 的一半仍开放，且开放得有道理**：staging 里每一条清理路径都是进程内的 defer（`source/scratch.go` 的构造失败回滚与 `Close`、`ownedtree.go` 的 reservation 清理），进程退出正因此留下残留，而这些名字的 identity 从未进入任何持久记录，故没有任何证据可据以证明归属——DESIGN 对 GC 的条件"在具备可验证所有权证据时"至今不成立。要让它成立须先为这些名字加持久 reservation，那是独立的一件事；
...
- **`fu revert` 未对并发的直接 git 写入做基线比对（第二十三轮记录）**：写流水线的 `run`（`pipeline.go`）在 sweep 之后逐字节比对基线，发现期间落地的直接 git 写入即以 `ErrConcurrentStoreChange` 中止；`revert` 没有等价物。`applyTreeToWorktree` 会先把工作区与目标树收敛，之后才取树指纹，因此指纹能发现目标未命名的内容，却发现不了目标已命名路径上的**不同**内容——更新器已经把它覆盖掉了。窗口很窄：`fu.lock` 排除其他 fu 进程，须是一次直接 git 写入恰好落在这段持锁区间内。选择记录而非关闭，是因为关闭意味着把 `run` 整套基线比对搬给 revert，超出本批范围；其代价是本文档「sweep 使 fu 这边一个字节都不丢」的说法就 revert 而言强于代码所能支撑的程度。`fu commit` 同此：它同样不经 `run`，候选内容持锁期间从磁盘冻结、发布以 CAS 保护引用，但没有 sweep 后的逐字节基线比对。
...
- **作用域提交的越界拒绝是候选基线选择的产物，而非独立设计（2026-09-02 评审记录）**：`PrepareCommitUnder` 为前缀之外的路径填入**公共 index** 的条目（`capturePublicIndex`），而候选的变更集是 HEAD→index 投影，故任何以直接 git 暂存于前缀之外、且与 HEAD 不同的路径都会落入变更集并触发包含性检查。SPEC §5.1 把它写成一条用户可理解的规则，是恰当的；但它并非先有的产品选择。改为从 HEAD 的 tree 取基线会使前缀外路径一律为 `Unmodified`，该拒绝随之消失，包含性检查回归真正的防御性断言，`Untracked` 特例（本分支两次修复的共同根因）亦不再需要。改动触及候选冻结这一共享路径，留作独立事项。**但按现状直接改基线会造成数据丢失，不可照搬**：作用域候选现无条件同步公共 index（§4 commit 行与 `commit_scope.go` 中 `PrepareCommitUnder` 末尾的同步一段），而 HEAD 基线会让前缀外路径以 HEAD 的版本进入候选 index，同步随即把用户以直接 git 暂存的内容悄悄回退且从不入账——今天挡住这一状态的正是那条越界拒绝，而该改动恰恰要移除它。故 HEAD 基线必须与「候选种子」和「同步基线」的分离一并设计。在其落地之前，该拒绝的存在有两重理由：本条所述的产物性，以及为无条件同步兜底；
- **每条 fu 写命令都会在公共 index 留下 `git add -N` 残留（2026-09-03 评审记录）**：根因是 go-git 的 index 模型不携带 git index 的若干标志位，写回时一律按其模型重建。已复现两种表现：其一，`doUpdateFileToIndex` 从不清除条目的 `IntentToAdd`，因此一个先经 `git add -N` 登记的路径在 fu 提交它之后，公共 index 中仍带该标志——`git status` 显示 `DA`，`git diff --cached` 报一处删除，而 `fu status` 静默；其二，`index.Entry` 没有 assume-valid 位（由 `git update-index --assume-unchanged` 设置）的字段，故任何 fu 写入公共 index 的操作都会清除它——已实测 `git ls-files -v` 由 `h fu.yaml` 变回 `H fu.yaml`。**这不是本分支引入的**：从 base 构建的二进制仅用 `fu new` 即可复现同一终态，故它属于每一条 fu 写命令，作用域提交只是与全库形式表现一致，而非另有偏差。根因在 go-git 而不在 fu，代价是与直接 git 并用时的一处显示偏差，无内容丢失；记录于此而非留在评审文件里，是因为下一个作者更可能先撞见症状。
...
- **退出码**：0 成功、1 操作失败、2 用法错误——用法错误包含五类：参数数量不对、未知 flag、**格式不合法的 flag 取值**（`--agent ""`、`--agent <无此适配器>`、显式空 `--ref`、不合法或不适用于来源类型的 `--ref`），**格式不合法的位置参数**（`fu revert abc` / `fu revert 0`、显式空名的 `fu update ""`），以及**flag 与位置参数的组合不合法**——取值与个数各自都合法，组合起来不是（`fu update --force` 不带 name：批量强制会在不点名任何一个的情况下覆盖多个 skill 的本地修改，故要求显式点名）。前两类由 `UsageError` 在 cobra 生成对应错误的原处构造（`SetFlagErrorFunc`、`Args` 校验器）；第四类与第五类同样在 `RunE` 内直接构造 `UsageError`：第四类见 `cli/revert.go`、`cli/update.go`（后者拒绝显式空名，`TestUpdateCommandRejectsAnExplicitlyEmptySkillName`）与 `cli/commit.go`（显式空名）；第五类见 `cli/update.go`——组合是否合法只看命令行本身，不必问 engine。第三类分两条路：`engine.ErrEmptyAgentScope`、`engine.ErrUnknownAgent`、`engine.ErrEmptyAddRef`、`engine.ErrInvalidAddRef` 在 `RunE` 内由 engine sentinel 判定后包装，因为「这个名字是否对应一个适配器」与来源/ref 组合是否合法只有 engine 知道；本轮新增的两处——`cli/commit.go` 的显式空 `-m` 与 `cli/log.go` 的非正 `-n`——则在 `RunE` 内直接构造，不经 engine sentinel，因为这两个取值是否合法只看命令行本身。`engine.ErrUpdateForceNeedsName` 不在第三类之列：它是 `Application` 这一层为 GUI 等其他前端保留的防御性兜底（`cli/update.go` 的注释说明该路径在本命令下今天不可达），CLI 走的是第五类那条自建 `UsageError`。对 `adopt`，已知但本机未检测到的 agent 是环境事实而非书写错误，计为操作失败、退出码 1；`enable` / `disable --agent` 则允许为这类 agent 预先持久化 override 并成功，待 agent 可用时生效。凡退出码 2 的路径都同时打印 usage（含未知子命令一路）；未知子命令的判定必须在 `root.Execute()` 返回之后，用 cobra 自身的只读 `Find` 复核——`help` / `completion` / `__complete` 等内建子命令要到 `Execute()` 内部才注册，提前判断会把它们也误判为未知命令；其余错误（含 `Reconcile` 因 `Result.Failed` 非空而返回的 `ErrOperationFailed`）一律计为操作失败。判定逻辑集中在 `internal/cli/exitcode.go` 的 `execute()`；
...
| 集成 | `t.TempDir()` 构造 FU_HOME 与假 agent 目录（覆盖 `HOME`） | 已交付十七条命令的读写路径（含 git 来源端到端、adopt 双形态、gc 裁剪边界及 status/restore/revert 的场景走查）。restore 收敛性覆盖链接层与 `--hard` 的工作区复位半层；revert 覆盖连续回退与脏工作区先 sweep 再回退两类场景 |
...
| 端到端 | 命令函数级联走查 | `scenario_test.go` 走查场景 7（含 `commit` 与 `log`）、场景 2 与场景 5 的 store 侧（含 `revert` 后的 `log` 一步），调用 engine API 而非 CLI。场景 1（安装）与场景 6（收编既有环境）自 plan 2 起已不依赖未交付命令，功能覆盖在 `add_test.go`、`adopt_test.go` 与 `adopt_whole_test.go`；场景 3（更新）自本轮起同样不再依赖未交付命令，功能覆盖在 `internal/cli` 的 `update_test.go`、`outdated_test.go` 与验收测试 `update_acceptance_test.go`。三者均尚未整理进这份场景走查——这是组织缺口而非测试缺口（SPEC §8 的验收标准是场景形态的，故记在此处）；仍依赖未交付命令的只剩场景 4（换机，待 `clone`） |
| 端到端 | 编译后二进制冒烟（go build + exec） | 1–2 条主路径，防"函数级可用、装配层坏"的盲区 |
| 故障注入 | 关键中断点模拟崩溃（`os.Args[0]` 派生子进程 + `os.Exit`，不跑 defer/回滚/Close） | 已交付：`new` 的两个所有权窗口、事务记录各阶段切换点、恢复内部六个边界、配置交换四个崩溃点；plan 2 追加：`add` 复制各窗口、`rm` 隔离各窗口、`adopt` 入库/切换/整目录交换各窗口；本轮追加：`update` 的五个边界——staging 创建后、交换前、交换后、commit 后（`TestUpdateCrashHelper`，`update_txn_test.go`），以及终态标记落地后、内联回收前（`beforeUpdateReclaim`，由 `gc_test.go` 派生的子进程驱动，覆盖的正是 `fu gc` staging 一侧存在的理由）。**工作区更新器（`applyTreeToWorktree`）刻意不用这套派生子进程注入，改以进程内中断覆盖**（第二十五轮记录）：`TestApplyTreeToWorktreeConvergesAfterAnInterruptedEntryLoop` 在条目循环中途停下，`TestResetWorktreeToHeadConvergesAfterAnInterruptedRun` 借持有 `.git/index.lock` 停在工作区已改写、index 未重建的那一步。这是形态之别而非覆盖之缺：派生子进程注入的价值在于跳过 defer/回滚/Close 以检验**崩溃点恢复**，而本更新器刻意无 WAL、也没有单独不安全的中间态，需要钉住的是"重跑即收敛"这条唯一的恢复叙事；唯一的原子性要求（index 经 `writePublicIndexAtomically` 以 rename 安装）另由 `TestRebuildIndexFromTargetInstallsTheIndexByRename` 直接钉住。记录于此，以免日后被读作漏做 |
...
| 崩溃恢复 | `new`/`rm` 按命令、`add`/`adopt` 按 skill 的不可变事务 revision 先于该项任何变更排他落盘（WAL 式），文件名提交本 revision 摘要且内容链接前一摘要，阶段只追加，完成标记绑定最新摘要 | 任意下一次写命令先验证完整连续链再到达定义终态；任何既有路径都不会被 WAL 更新覆盖或在完成时删除，现场残留仅在匹配记录时处理（早期设想的 `.old` 形状未采用，见 §6）；保证覆盖进程崩溃，不覆盖掉电——原子写未对父目录 fsync，rename 能否在掉电后存活不受保证 |
